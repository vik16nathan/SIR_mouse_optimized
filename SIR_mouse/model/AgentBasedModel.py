...
import numpy as np #CHANGE THIS: convert this all to cython for faster execution? could be a fun side project
...
class AgentBasedModel:
    def __init__(
        ...
        self.trans_rate = 1 / self.region_size #CHANGE THIS for voxel-wise - test model assumptions
    ...
    def update_spread_process(self, v_scale=1, spread_scale=1): #CHANGE THIS: test parameters to speed up/slow down spreading? What is v_scale? ask Shady
        """
    ...
    def update_growth_process(self, growth_scale=1): #CHANGE THIS: NOT USED
    ...
    def s_spread_step(self):
        ...
        self.s_region += edge_to_region.sum(axis=0) - \
            np.append(  #CHANGE THIS: could pre-initialize instead of having to append every time (slow) - test out with os.time
    ...
    def trans_step(self): #CHANGE THIS: bigger region = more likely to switch from S --> I (strange assumption)
